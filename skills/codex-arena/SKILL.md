...
This adjudication step needs an LLM to judge "is this factually wrong" — `arena.sh --evaluator codex` with no Anthropic key available has no LLM in the loop besides Codex itself, just bash resubmitting Codex's own proposal as a hypothetical, so it can't contest a claim on its own; without a paid API key there is no other model an unattended script can reach, so Codex necessarily judges and fixes itself. It has a real equivalent once a key is available though (checked automatically, no flag — same `ANTHROPIC_API_KEY` path `--evaluator claude` uses), and once it's found, Codex becomes evaluator-only, the same rule `--evaluator claude` already follows in reverse: the evaluator judges, it never writes the accepted fix. Codex critiques each round; Claude either writes its own corrected version (shown Codex's own proposed fix only as context, never used directly) or disputes the finding, putting the objection back to Codex to withdraw or defend — the same mechanics this section describes, just automated instead of Claude Code doing the judging live. This only activates when a key is already present (env var, or cached from a prior `--evaluator claude` run) — it never triggers the interactive key-setup prompt itself, so a plain default run stays free and non-interactive unless a key already exists.
